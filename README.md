...
1. Open **Settings → LanTai → Storage**.
...
5. Open the gallery from the ribbon to browse recent, S3, and local images.
...
- **Migration wizard (desktop)** — Upload local images referenced in selected note folders to the active profile, rewrite links, and resume from a saved plan. Not shown on mobile.
...
Supports AWS S3, Cloudflare R2, Alibaba Cloud OSS, Tencent Cloud COS, and S3-compatible endpoints such as MinIO; optional delete of the local source after a successful upload; Wiki or Markdown for local links (remote http(s) URLs always use Markdown).
...
- The migration wizard is **desktop only**; unfinished plans wait until you open the vault on desktop.
